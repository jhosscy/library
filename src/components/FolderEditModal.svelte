<script>
  let { id, title: propTitle, icon, color } = $props();
  import { updateSubject } from '../services/subjectServices.ts';
  ...
  async function updateSubjectHandler(e) {
    ...
    if (subject) {
      ...
      document.querySelector("#subject-title").innerText = title.trim();
      const titleNow = window.location.href.split("/").at(-1).split("-")[0]
      const newURL = window.location.href.replace(titleNow, title.trim());
